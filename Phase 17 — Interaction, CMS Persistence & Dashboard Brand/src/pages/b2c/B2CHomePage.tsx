...
export default function B2CHomePage() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[var(--color-bg)]"><PublicHeader /><main>
    ...
    <section className="mx-auto grid max-w-[1440px] gap-6 px-4 py-14 md:px-16 md:py-20 lg:grid-cols-2"><article className="relative min-h-[360px] overflow-hidden rounded-[28px] bg-[var(--color-text-muted)] p-7 text-[var(--color-surface)] md:p-10">{content.businessImageUrl?<img src={content.businessImageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-35"/>:null}<div className="relative z-10"><span className="text-xs font-semibold text-[var(--color-accent)]">{content.businessEyebrow}</span><h2 className="mt-3 text-3xl font-bold">{content.businessTitle}</h2><p className="mt-4 max-w-lg text-sm leading-7 opacity-90">{content.businessDescription}</p><Link to="/business" className="mt-7 inline-flex rounded-xl bg-white px-5 py-3 text-sm font-bold text-[#2E1E17] shadow-sm transition-colors hover:bg-[#F7F0E8]">الدخول إلى قسم الشركات</Link></div></article><article className="relative min-h-[360px] overflow-hidden rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-7 md:p-10">{content.aboutImageUrl?<img src={content.aboutImageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-20"/>:null}<div className="relative z-10"><span className="text-xs font-semibold text-[var(--color-accent)]">{content.aboutEyebrow}</span><h2 className="mt-3 text-3xl font-bold">{content.aboutTitle}</h2><p className="mt-4 text-sm leading-7 text-[var(--color-text-muted)]">{content.aboutDescription}</p><Link to="/about" className="mt-7 inline-flex text-sm font-bold text-[var(--color-text-muted)] underline underline-offset-4">اعرف أكثر</Link></div></article></section>

    <section className="border-t border-[var(--color-border)] bg-[var(--color-surface)]"><div className="mx-auto flex max-w-[1440px] flex-col gap-6 px-4 py-12 text-right md:flex-row md:items-center md:justify-between md:px-16"><Link to="/contact" className="inline-flex h-11 items-center justify-center rounded-xl border border-[#2E1E17]/25 bg-white px-6 text-sm font-semibold !text-[#2E1E17] hover:bg-[#F7F0E8]">تواصل معنا</Link><div><h2 className="text-2xl font-bold">{content.contactTitle}</h2><p className="mt-2 text-sm text-[var(--color-text-muted)]">{content.contactDescription}</p></div></div></section>

...
export default function PaymentWorkflowPanel({ request, busy, onSave }: Props) {
  ...
  // NOT_RECORDED is the initial persisted status, not a completed payment.
  const awaitingPayment = request.payment_status === 'NOT_RECORDED' || request.payment_status === 'PENDING'
  const alreadyProcessed = !awaitingPayment
